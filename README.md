...
Agentic SDLC 全流程智能体 - 以规格（Spec）为中心的八环节流水线（需求→发布）；**问题单（bug）修复走 5 步流程**（定位 / 根因 / 修复 / 验证 / 交付，不走八环节）。这是本仓库的**第一个 Agent**（子智能体），区别于 Ga/Gamk 的 Skill/Command。
...
当需要**端到端实现一个需求或功能**、走完整软件开发生命周期时，主 Claude 会自动委派给 SDLC 智能体（也可显式 `@sdlc` 或要求「走 SDLC 全流程」）；修复**问题单（bug）**时也用本智能体——走 5 步流程，不走八环节。S1~S4 产物统一放在**同一需求专属文件夹**内：

| 环节 | 产物 | 门禁 |
|------|------|------|
| S1 需求分析 | `<需求文件夹>/`（`analysis.md` 或入口文件 S1 节） | 人工门禁 |
| S2 需求设计（Spec） | `<需求文件夹>/`（`spec.md` 或入口文件 S2 节） | **人工签署** ⚑ |
| S3 软件设计 | `<需求文件夹>/`（`design.md` 或入口文件 S3 节；ADR 优先内联） | 人工门禁 |
| S4 任务分解编排 | `<需求文件夹>/`（`tasks.md` 或入口文件 S4 节） | 自动 + 抽查 |
...
问题单开发流程智能体（轻量三段式）- 把一个 Jira 问题单从**根因**推进到**修复落地 + 自测**，全程只产一个文件。与 SDLC 互补：SDLC 覆盖「需求八环节 + 问题单 5 步」全流程，Issue-Fix 是问题单的**轻量三段式**（单文件产出自测报告，适合小修快修）；上游「问题单分析技能」产出根因后由它接力。
